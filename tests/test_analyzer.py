...
class TestOpenAIAnalyzer(unittest.TestCase):
    ...
    @patch("platzi_news.analysis.analyzer.AsyncOpenAI")
    def test_analyze_success(self, mock_openai):
        ...
        answer = asyncio.run(analyzer.aanalyze(articles, "What is this about?"))
        self.assertEqual(answer, "Test answer")

    @patch("platzi_news.analysis.analyzer.AsyncOpenAI")
    def test_analyze_no_articles(self, mock_openai):
        analyzer = OpenAIAnalyzer("fake_key")
        answer = asyncio.run(analyzer.aanalyze([], "Question"))
    ...
    def test_analyze_error(self, mock_openai):
        ...
        with self.assertRaises(AnalysisError):
            asyncio.run(analyzer.aanalyze(articles, "Question"))
